...
#df1 = pd.read_excel("Panel Member_March.xlsx")
#df2 = pd.read_excel("Panel Memebers_AZE channels.xlsx")
...
df1_grouped = df1.groupby(by=["Household"])['Hours', "Minutes", "Seconds"].sum()
...
df2_grouped = df2.groupby(by=["Household"])['Hours', "Minutes", "Seconds"].sum()
...
print(df2_grouped.head())


df1_grouped.to_excel(r'total.xlsx')
df2_grouped.to_excel(r'total.xlsx')